...
public abstract class AbstractDoxygenMojo 
	...
	/**
	 * This defines the name and/or the absolute path
	 * to he doxygen executable.
	 * Usually you shouldn't change this, cause the plugin will
	 * automatically search for the executable.
	 * 
    ...
    /**
     * This method will get an existing configuration file 
     * for doxygen or will create a new one with the configured
     * parameters.
     ...
     * Furthermore is will put all configurable items into the 
     * configuration file.
     * 
    ...
    public void buildConfigurationFile(File config)
        ...
        try {
            ...
            throw new MavenReportException(
                "Error creating Doxygen configuration file '"
                    + config.getAbsolutePath() + "'.", ex);
        } finally {
            if (out != null) {
                out.close();
            }
    ...
    /**
     * This will write all kind of configuration items into the configuration
     * file and will format the file.
